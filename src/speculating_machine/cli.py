...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Speculating-Machine thinks for you about the meanings.",
        epilog="Example:  speculating-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | speculating-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | speculating-machine         #

    $ speculating-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="speculating-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"speculating-machine: {utterance}.")
        syslog.closelog()
